...
def write_to_xml(midi_object, filename):
    ...
    # Ensure timeSignatures has at least one item or use a default time signature
    timeSignatures = song.time_signature_changes
    if not timeSignatures:
        from pretty_midi import TimeSignature
        default_time_signature = TimeSignature(4, 4, 0)  # Default to 4/4 at the start
        timeSignatures = [default_time_signature]
    #list containing key signature of piece, as well as time stamp when key signature changes
    # Ensure keySignatures has at least one item or use a default key signature
    keySignatures = song.key_signature_changes
    if not keySignatures:
        from pretty_midi import KeySignature
        default_key_signature = KeySignature(0, 0)  # C major or A minor (no accidentals)
        keySignatures = [default_key_signature]
...
midi_path = '../../client/public/output.mid'
midi_object = pretty_midi.PrettyMIDI(midi_path)

# Now pass the PrettyMIDI object to write_to_xml
write_to_xml(midi_object, 'output.musicxml')